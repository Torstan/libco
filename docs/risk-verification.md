...
| --- | --- | --- | --- | --- | --- |
| P0-HOOK-FD-RACE | P0 | Global hook fd table race and UAF | `scripts/risk/run_tsan.sh` | needs environment | `timeout 60s scripts/risk/run_tsan.sh` exit 1; `logs/risk/P0-HOOK-FD-RACE.tsan.log`: hooked socket creation failed in sandbox |
...
| P1-ENV-LEAK | P1 | Coroutine private environment leak | `scripts/risk/run_asan_lsan.sh` | needs environment | `timeout 30s make risk-diagnose`: env probe completed without leak report; `logs/risk/P1-leaks.asan-lsan.log`: LeakSanitizer fatal/runtime limitation |
...
| P1-COND-CROSS-THREAD | P1 | `CoCond` cross-thread signal | `scripts/risk/run_tsan.sh` | confirmed | `timeout 60s scripts/risk/run_tsan.sh` exit 1; `logs/risk/P1-COND-CROSS-THREAD.tsan.log`: TSan data race in `CoCond::Signal()`/`CoCond::Timedwait()` |
