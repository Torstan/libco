...
| --- | --- | --- | --- | --- | --- |
| P0-HOOK-FD-RACE | P0 | Global hook fd table race and UAF | `scripts/risk/run_tsan.sh` | needs environment | `timeout 60s scripts/risk/run_tsan.sh` exit 1; rebuilt in `test/risk/build-tsan`; `logs/risk/P0-HOOK-FD-RACE.tsan.log`: hooked socket creation failed in sandbox |
...
| P0-POLL-SAME-FD | P0 | Two coroutines poll the same fd | `make risk-check`; `scripts/risk/run_strace_poll.sh` | confirmed | `make risk-check`; `timeout 60s scripts/risk/run_strace_poll.sh` exit 1; `logs/risk/P0-POLL-SAME-FD.strace.log`: structured `needs environment` record for ptrace denied |
...
| P1-ENV-LEAK | P1 | Coroutine private environment leak | `scripts/risk/run_asan_lsan.sh` | needs environment | `timeout 60s scripts/risk/run_asan_lsan.sh` exit 1; rebuilt in `test/risk/build-asan`; `logs/risk/P1-leaks.asan-lsan.log`: sanitizer detector found UBSan runtime errors and ASan SEGV before leak-only conclusion |
| P1-THREADENV-LEAK | P1 | Per-thread `ThreadEnv` leak | `scripts/risk/run_asan_lsan.sh` | confirmed | `timeout 30s make risk-diagnose`: `fd_count_before=7 fd_count_after=57`; `logs/risk/P1-leaks.asan-lsan.log` |
...
| P2-PLATFORM-GUARD | P2 | Platform and ABI guard | manual inspection | documented boundary | `timeout 30s make risk-diagnose`: diagnostic notes `-m64` and architecture guard inspection boundary |
